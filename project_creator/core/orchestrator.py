...
class Orchestrator:
    def __init__(self, router, agents, storage, tools, manifest, session):
        ...
        self.repair_strategist = RepairStrategist(
            agents["repair"], self.error_classifier, storage.project_root
        )
        self.validation_manager = ValidationManager(
            agents, tools, self.repair_strategist
        )
        self.gen_coordinator = GenerationCoordinator(
            self.coder, self.validation_manager
        )
    ...
    def generate_with_dependency_order(self):
        ...
        for path in order:
            if path in meta_map:
                ...
                # P0-4: Prevent manual_review_needed files from being applied automatically
                if res.get("status") == "manual_review_needed":
                    print(
                        f"⚠️  Orchestrator: {path} requires manual review and will not be applied automatically."
                    )
                    continue
    ...
    def validate_dependencies(self):
        ...
        return self.dependency_analyzer.analyze_project(
            self.blueprint["files"] if self.blueprint else []
        )
    ...
    def plan(self, goal: str):
        ...
        self.blueprint = self.planner.create_blueprint(
            goal, strategy_doc=self.strategy_doc
        )
        if self.blueprint:
    ...
    def generate_and_validate(self, file_meta: Dict[str, str]):
        strategy_doc = getattr(self, "strategy_doc", None)
        return self.gen_coordinator.generate_project_file(
            file_meta, self.blueprint, self.generated_files, strategy_doc=strategy_doc
        )
    ...
    def apply(self, path: str, content: str):
        ...
        if LearningConstitution.has_forbidden_content(content):
            logging.warning(
                f"SECURITY: Blocked write to {path} due to forbidden patterns (Secrets/Injection)"
            )
            print(
                f"🛡️  Orchestrator: Blocked write to {path} due to forbidden patterns (Secrets/Injection)"
            )
            return False
        ...
        if self.storage.write_file(path, content, interactive=False):
            ...
            self.session.save_session(
                ...
                self.generated_files,
                [],
                [p for p in self.generated_files.keys()],
            )
            # Learning Event
