...
class RepairStrategist:
    ...
    def formulate_repair(self, path: str, content: str, issues: list, blueprint: Dict, generated_files: Dict, extra_context: str = None):
        ...
        impact_warning = f"\n⚠️  WARNING: Changes to {path} may affect: {', '.join(impacted)}" if impacted else ""

        # 3. Combine classifier plan with agent proposing
        augmented_context = f"REPAIR STRATEGY: {plan}{impact_warning}\n\n{extra_context}" if extra_context else f"REPAIR STRATEGY: {plan}{impact_warning}"
