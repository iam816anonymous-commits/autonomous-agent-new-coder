...
class CommitLearner:
    ...
    def learn_history(self, limit=50):
        print(f"🕰️  Reality Learning: Analyzing last {limit} commits in {self.repo_path}")
        try:
            # Get commit hashes
    ...
    def _analyze_commit(self, commit_hash: str):
        try:
            ...
            meta_res = subprocess.run(cmd, cwd=self.repo_path, capture_output=True, text=True)
            if not meta_res.stdout: return
            ...
            files_res = subprocess.run(cmd, cwd=self.repo_path, capture_output=True, text=True)
