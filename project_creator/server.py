...
# Global state for simplicity in this bridge
class SessionState:
    def __init__(self):
        # Delay initialization until needed to avoid environment variable checks during startup
        self._router = None
        self._planner = None
        self._coder = None
...
async def generate_blueprint(req: GoalRequest):
    ...
    if not state.blueprint:
        raise HTTPException(status_code=500, detail="Failed to plan")
...
@app.post("/critique")
async def critique_file(path: str, content: str):
    critique = state.critique_agent.analyze(path, content, state.blueprint, state.generated_files)
    return {"path": path, "critique": critique}

@app.post("/apply")
async def apply_file(path: str, content: str):
