...
async def get_recent_activity(limit: int = 5):
    ...
    try:
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            cursor.execute(
                "SELECT 'PATTERN' as type, pattern_type as label, content, last_seen as ts FROM patterns ORDER BY last_seen DESC LIMIT ?",
                (limit,),
            )
            patterns = [dict(row) for row in cursor.fetchall()]

            cursor.execute(
                "SELECT 'CODE' as type, status as label, file_path as content, created_at as ts FROM snippets ORDER BY created_at DESC LIMIT ?",
                (limit,),
            )
...
async def update_pattern(pattern_id: int, content: str = Body(..., embed=True)):
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE patterns SET content = ? WHERE id = ?", (content, pattern_id)
            )
            conn.commit()
            return {"status": "ok"}
    except Exception as e:
...
async def export_lora(
    output_path: str = Body(default="lora_dataset.jsonl", embed=True)
):
...
async def search_docs(query: str):
    ...
    try:
        ...
        result = router.gemini.generate(
            f"Search for documentation: {query}", enable_search=True
        )
...
async def get_active_context(path: str):
    ...
    try:
        ...
        return {"semantic": semantic, "patterns": patterns}
    except Exception as e:
        return {"error": str(e)}
